				...
				taskGroup.map(async (element) => {
					try {
						...
						if (endDate) {
							event = {
								summary: name,
								start: startDate.length === 10 ? { date: startDate } : { dateTime: startDate },
								end: endDate.length === 10 ? { date: endDate } : { dateTime: endDate },
								location,
							};
						}
						...
						try {
							...
							if (!eventIdText) {
								eventResponse = await makeGoogleAPIRequestWithRetry<GoogleCalendarEvent>(calendarUrl, 'POST', env, event);
							} else if (!inTrash) {
								...
								eventResponse = await makeGoogleAPIRequestWithRetry<GoogleCalendarEvent>(`${calendarUrl}/${eventIdText}`, 'DELETE', env);
							}
